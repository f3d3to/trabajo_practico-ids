    ...
    if request.method == 'GET':
        try:
            ...
            if response.status_code == 200 and response.json().get("success"):
                ...
                print("Error al obtener datos de la mascota.")
                mascota = {"id": id}  # Valores predeterminados si falla la solicitud
        except requests.exceptions.RequestException as e:
            print(f"Error de conexión al backend: {e}")
            return render_template("error.html", error="Error de conexión con el backend.")
        ...
        try:
            ...
            if esquema_response.status_code == 200 and esquema_response.json().get("success"):
                ...
                if 'foto' in request.files:
                    ...
                    if foto.filename:  # Si se subió una imagen
                        try:
                            files = {'file': (foto.filename, foto.stream, foto.content_type)}
                            upload_response = requests.post(BACKEND_URL + "/upload", files=files)
                            if upload_response.status_code == 201:
                                mascota_data['foto_url'] = foto.filename
                        except requests.exceptions.RequestException as e:
                ...
                if response.status_code == 200 and response.json().get("success"):
                    ...
                    return redirect(url_for('detalleMascota', id=id))
                else:
                    print("Error al actualizar la mascota:", response.json())
                    return render_template("actualizarMascota.html", mascota=mascota_data, error="No se pudo actualizar la mascota.")
        except requests.exceptions.RequestException as e:
            print(f"Error de conexión al backend: {e}")
            return render_template("actualizarMascota.html", error="Error de conexión con el backend.")
